...
x3min = -5
x3max = 10
#якщо не адекватно множимо вхідні значення по варіанту на 1.8 та починаємо з початку
while True:
        x01 = (x1max + x1min)/2
        x02 = (x2max + x2min)/2
        x03 = (x3max + x3min)/2
        ...
        if Fp>Ft:
            print("Fp=",round(Fp,2),">Ft",Ft,"\nРівняння неадекватно оригіналу, тому починаємо з початку та домножаємо вхідні значення на 1.8 ")
            x1min *= 1.8
            x1max *= 1.8
            x2min *= 1.8
            x2max *= 1.8
